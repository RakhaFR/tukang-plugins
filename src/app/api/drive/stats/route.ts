...
export async function GET() {
  try {
    ...
    const popularPlugins = displayFiles.slice(0, 6).map((file: any, index: number) => {
      ...
      // Rumus Download yang Lebih Realistis (Hanya puluhan sampai ratusan unduhan)
      ...
      return {
        ...
        dl: `${calculatedDownloads}`, // Mengembalikan string angka murni (misal: "45")
        rawSize: formattedSize, // Ukuran asli dari Drive
        webViewLink: file.webViewLink || '#'
      };
    });
